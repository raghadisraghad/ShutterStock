...
const getAll = asyncHandler(async (req, res) => {
  const target = await Target.find().populate('client').populate('product').populate('video').populate('audio');
  res.status(200).json(target);
});

const getById = asyncHandler(async (req, res) => {
  const { id } = req.params
  const target = await Target.findById(id).populate('client').populate('product').populate('video').populate('audio');
  if (!target) {
...
const getClientOrder = asyncHandler(async (req, res) => {
  const { id } = req.params
  const target = await Target.find({ client: id }).populate('client').populate('product').populate('video').populate('audio');
  if (!target) {
    return res.status(404).send({ error: 'Target Not Found!' });
  }
  res.status(200).json(target);
