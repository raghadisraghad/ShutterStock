...
const deleteC = asyncHandler(async (req, res) => {
    ...
    const uploadServiceDirectory = path.join(process.env.UPLOAD_DIRECTORY, 'Services');
    const userDirectory = path.join(uploadProductDirectory, target.username.toString());
    const userDirectory2 = path.join(uploadServiceDirectory, target.username.toString());
    ...
    if (fs.existsSync(userDirectory2)) {
        if (fs.existsSync(userDirectory2)) {
            fs.rm(userDirectory2, { recursive: true, force: true }, (err) => {
                if (err) {
                    console.error('Error deleting folder:', err);
                    return;  
                }
    ...
    const DeleteUser = await Target.findByIdAndDelete(id);
